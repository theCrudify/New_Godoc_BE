...
export const getAllDepartmentHeads = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const [sectionHeads, totalCount] = await prismaDB2.$transaction([
            ...
            prismaDB2.mst_department_head.count({ where: whereCondition }),
        ]);
...
export const getAllDepartmentHeadsbyID = async (req: Request, res: Response): Promise<void> => {
    try {
        const page = Number(req.query.page) || 1;
        const limit = Number(req.query.limit) || 10;
        const searchTerm = (req.query.search as string) || "";  // Tetap sediakan searchTerm umum
        const idSearchTerm = req.query.id as string; // Parameter pencarian khusus ID
        const status = req.query.status === "true";
        const sortColumn = (req.query.sort as string) || "id";
        const sortDirection = req.query.direction === "desc" ? "desc" : "asc";
        ...
        if (idSearchTerm) {
            ...
            whereCondition.id = Number(idSearchTerm) || 0; // Konversi ke number, default 0 jika NaN
        } else if (searchTerm) {
            //Jika tidak ada pencarian berdasarkan id, maka lakukan pencarian umum
            whereCondition.OR = [
                { created_by: { contains: searchTerm, mode: "insensitive" } },
                { authorization: { employee_code: { contains: searchTerm, mode: "insensitive" } } },
                { authorization: { employee_name: { contains: searchTerm, mode: "insensitive" } } },
...
export const createDepartmentHead = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        // Cek apakah kombinasi `authorization_id` dan `section_id` sudah ada di `mst_department_head`
        const existingSectionHead = await prismaDB2.mst_department_head.findFirst({
        ...
        if (existingSectionHead) {
            if (existingSectionHead.is_deleted) {
                ...
                const restoredSectionHead = await prismaDB2.mst_department_head.update({
        ...
        // Simpan data baru ke `mst_department_head`
        const newSectionHead = await prismaDB2.mst_department_head.create({
            data: {
                authorization_id,
                section_id,
                created_by,
...
export const softDeleteDepartmentHead = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const existingData = await prismaDB2.mst_department_head.findUnique({
            where: { id }
        });
        ...
        await prismaDB2.mst_department_head.update({
            where: { id },
...
export const updateDepartmentHead = async (req: Request, res: Response): Promise<void> => {
    try {
        const id = Number(req.params.id);
        ...
        if (section_id) {
            ...
            const duplicateSectionHead = await prismaDB2.mst_department_head.findFirst({
                where: {
                    section_id,
                    is_deleted: false,
        ...
        const updatedSectionHead = await prismaDB2.mst_department_head.update({
            where: { id },
            data: updateData
        });
