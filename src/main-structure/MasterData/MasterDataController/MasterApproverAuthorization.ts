...
interface TemplateApprovalAuthData {
...
interface TemplateAuthWhereCondition {
...
// Fungsi validasi input untuk mst_template_approval_authorization
function validateTemplateApprovalAuthData(data: any): string[] {
    const errors: string[] = [];
...
// Get all template approval authorizations with pagination and search
export const getAllTemplateApprovalAuthorizations = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const whereCondition: TemplateAuthWhereCondition = {
            is_deleted: false
        };
        ...
        const [templateAuthorizations, totalCount] = await prismaDB2.$transaction([
            prismaDB2.mst_template_approval_authorization.findMany({
            ...
            prismaDB2.mst_template_approval_authorization.count({
                where: whereCondition
            })
        ]);

        // Format the response data
        const formattedData = templateAuthorizations.map(item => ({
            ...item,
        ...
        console.error("❌ Error fetching template approval authorizations:", error);
        res.status(500).json({ 
...
export const getTemplateApprovalAuthorizationById = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const templateAuthorization = await prismaDB2.mst_template_approval_authorization.findFirst({
            where: {
                id: id,
                is_deleted: false
            },
        ...
        const formattedData = {
            ...
            applies_to_lines: formatAppliesToLines(templateAuthorization.applies_to_lines)
        };

        res.status(200).json({ data: formattedData });

    } catch (error) {
        console.error("❌ Error fetching template approval authorization by ID:", error);
        res.status(500).json({ 
            error: "Internal Server Error", 
            details: error instanceof Error ? error.message : "Unknown error" 
        });
...
// Create new template approval authorization
export const createTemplateApprovalAuthorization = async (req: Request, res: Response): Promise<void> => {
    try {
        const validationErrors = validateTemplateApprovalAuthData(req.body);
        if (validationErrors.length > 0) {
            res.status(400).json({ errors: validationErrors });
            return;
        }

        const data: TemplateApprovalAuthData = req.body;
        const parsedAppliesToLines = parseAppliesToLines(data.applies_to_lines);

        const newTemplateAuthorization = await prismaDB2.$transaction(async (tx) => {
            // Check for duplicate
            const existingTemplate = await tx.mst_template_approval_authorization.findFirst({
                where: {
            ...
            // Create new record
            return await tx.mst_template_approval_authorization.create({
                data: {
                    template_name: data.template_name,
                    line_code: data.line_code || null,
                    step_order: data.step_order,
        ...
        const formattedData = {
            ...newTemplateAuthorization,
            applies_to_lines: formatAppliesToLines(newTemplateAuthorization.applies_to_lines)
        };

        res.status(201).json({
            message: "Template approval authorization created successfully",
            data: formattedData
        });
        ...
        if (error instanceof Error && error.message === "DUPLICATE_TEMPLATE") {
            res.status(409).json({ 
                error: "Template approval authorization with same name, step order, model type, and line code already exists" 
            });
        } else {
            console.error("❌ Error creating template approval authorization:", error);
            res.status(500).json({ 
                error: "Internal Server Error", 
                details: error instanceof Error ? error.message : "Unknown error" 
            });
...
// Update template approval authorization
export const updateTemplateApprovalAuthorization = async (req: Request, res: Response): Promise<void> => {
    try {
        const id = Number(req.params.id);
        ...
        const duplicateTemplate = await prismaDB2.mst_template_approval_authorization.findFirst({
            where: {
                template_name: data.template_name,
                step_order: data.step_order,
                model_type: data.model_type as any,
        ...
        const updatedTemplateAuthorization = await prismaDB2.mst_template_approval_authorization.update({
            where: { id: id },
            data: {
                template_name: data.template_name,
                line_code: data.line_code || null,
        ...
        // Format response
        const formattedData = {
            ...updatedTemplateAuthorization,
            applies_to_lines: formatAppliesToLines(updatedTemplateAuthorization.applies_to_lines)
        };

        res.status(200).json({
            message: "Template approval authorization updated successfully",
            data: formattedData
        });

    } catch (error) {
        console.error("❌ Error updating template approval authorization:", error);
        res.status(500).json({ 
            error: "Internal Server Error", 
            details: error instanceof Error ? error.message : "Unknown error" 
        });
...
// Soft delete template approval authorization
export const deleteTemplateApprovalAuthorization = async (req: Request, res: Response): Promise<void> => {
    try {
        const id = Number(req.params.id);
        ...
        // Soft delete
        await prismaDB2.mst_template_approval_authorization.update({
            where: { id: id },
            data: {
                is_deleted: true,
                is_active: false,
        ...
        res.status(200).json({
            message: "Template approval authorization deleted successfully"
        });

    } catch (error) {
        console.error("❌ Error deleting template approval authorization:", error);
        res.status(500).json({ 
            error: "Internal Server Error", 
            details: error instanceof Error ? error.message : "Unknown error" 
        });
...
// Bulk create template approval authorizations
export const bulkCreateTemplateApprovalAuthorizations = async (req: Request, res: Response): Promise<void> => {
    try {
        const { data: templateAuthorizations } = req.body;

        if (!Array.isArray(templateAuthorizations) || templateAuthorizations.length === 0) {
        ...
        await prismaDB2.$transaction(async (txRaw) => {
            ...
            for (let i = 0; i < templateAuthorizations.length; i++) {
                try {
                    const validationErrors = validateTemplateApprovalAuthData(templateAuthorizations[i]);
                    if (validationErrors.length > 0) {
                        failures.push({ 
                            index: i, 
                    ...
                    const data: TemplateApprovalAuthData = templateAuthorizations[i];
                    ...
                    const result = await tx.mst_template_approval_authorization.create({
                        data: {
                            template_name: data.template_name,
                            line_code: data.line_code || null,
                            step_order: data.step_order,
        ...
        if (failures.length === 0) {
            res.status(201).json({
                message: `${successes.length} template approval authorizations created successfully`,
                data: successes
            });
        } else {
            res.status(207).json({
                message: `${successes.length} template approval authorizations created, ${failures.length} failed`,
                successes,
                failures
            });
        }
        ...
        console.error("❌ Error bulk creating template approval authorizations:", error);
        res.status(500).json({ 
            error: "Internal Server Error", 
            details: error instanceof Error ? error.message : "Unknown error" 
        });
...
// Get template approval authorizations by template name
export const getTemplateApprovalAuthorizationsByTemplateName = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const templateAuthorizations = await prismaDB2.mst_template_approval_authorization.findMany({
            where: {
        ...
        console.error("❌ Error fetching template approval authorizations by template name:", error);
        res.status(500).json({ 
            error: "Internal Server Error", 
            details: error instanceof Error ? error.message : "Unknown error" 
        });
...
// Get template approval authorizations by line code (including insert steps)
export const getTemplateApprovalAuthorizationsByLineCode = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        console.error("❌ Error fetching template approval authorizations by line code:", error);
        res.status(500).json({ 
            error: "Internal Server Error", 
            details: error instanceof Error ? error.message : "Unknown error" 
        });
