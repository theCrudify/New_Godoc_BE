...
// Add this new function to handle "submitted" notifications specifically
async function sendSubmissionEmails(proposedChangesId: number, authId: number): Promise<void> {
    try {
        // Get the proposed change data with document number and approvals
        ...
        const firstApprover = proposedChange.approvals.find(a => a.status === 'on_going');
        if (!firstApprover || !firstApprover.authorization || !firstApprover.authorization.email) {
            console.log(`No valid first approver found for proposed change ${proposedChangesId}`);
            return;
        }

        // Get document number
        const runningNumber = proposedChange.documentNumber?.running_number || 'N/A';
        ...
        const submitterEmailSubject = `[Go-Document] Dokumen Berhasil Diajukan: ${proposedChange.project_name}`;
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          ...
          <p>Dengan hormat,</p>
          <p>Dokumen proyek <strong>${proposedChange.project_name}</strong> - ( <strong>${runningNumber}</strong>) telah berhasil diajukan dan sedang menunggu persetujuan.</p>
    ...
    `;
        ...
        const approverEmailSubject = `[Go-Document] Dokumen Menunggu Persetujuan Anda: ${proposedChange.project_name}`;
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          ...
          <p>Dengan hormat,</p>
          <p>Dokumen proyek <strong>${proposedChange.project_name}</strong> - ( <strong>${runningNumber}</strong>) telah diajukan dan <strong>menunggu persetujuan Anda</strong>.</p>
