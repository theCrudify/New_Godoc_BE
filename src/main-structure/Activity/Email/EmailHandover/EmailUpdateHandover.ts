...
// Function to send notification emails about handover doc updates
async function sendHandoverUpdateNotificationEmails(
  handoverId: number,
  existingRecord: any,
  ...
  try {
      ...
      // Get the document number directly from the handover record
      const docNumber = existingRecord.doc_number || "NO-DOC-NUMBER";
      const projectName = existingRecord.tr_proposed_changes?.project_name || "Handover Document";
      ...
      if (submitter?.email) {
          ...
          const emailSubject = `[Go-Document] Dokumen Handover Telah Diperbarui: ${projectName}`;

          const submitterTemplate = createHandoverUpdateNotificationTemplate(
              'submitter',
              {
                  handover: existingRecord,
                  submitter: submitter,
                  proposedChange: existingRecord.tr_proposed_changes
              },
              docNumber,
              submitterGender
          );
          ...
          console.log(`✅ Handover update notification email sent to submitter: ${submitter.email}`);
      } else {
      ...
      for (const approver of notApprovedApprovers) {
          if (approver.email) {
              ...
              const approverTemplate = createHandoverUpdateNotificationTemplate(
                  'approver',
                  {
                      handover: existingRecord,
                      submitter: submitter,
              ...
              console.log(`✅ Handover update notification email sent to approver: ${approver.email}`);
          } else {
      ...
      console.error("❌ Error sending handover update notification emails:", error);
...
// Function to create email template for handover update notifications
function createHandoverUpdateNotificationTemplate(
  ...
  const projectName = data.proposedChange?.project_name || "Handover Document";
  ...
  switch (recipientType) {
      case 'submitter':
          ...
          bodyContent = `
              <p>Dengan hormat,</p>
              <p>Kami ingin memberitahukan bahwa dokumen handover <strong>${projectName}</strong> - ( <strong>${docNumber}</strong>) yang Anda buat telah diperbarui dalam sistem kami.</p>
              <p>Dokumen telah direvisi dan siap untuk diproses persetujuan kembali.</p>
          `;
          buttonHtml = `
              <div class="button-container">
                  <a href="http://localhost:4200/activity-page/handover-detail/${data.handover.id}" class="button">Lihat Detail Dokumen</a>
              </div>
          `;
          break;
      case 'approver':
          ...
          bodyContent = `
              <p>Dengan hormat,</p>
              <p>Kami ingin memberitahukan bahwa dokumen handover <strong>${projectName}</strong> - ( <strong>${docNumber}</strong>) yang sebelumnya Anda <strong>TIDAK SETUJUI</strong> telah diperbarui oleh submitter.</p>
          ...
          buttonHtml = `
              <div class="button-container">
                  <a href="http://localhost:4200/activity-page/handover-approval-detail/${data.handover.id}" class="button">Review & Approve Dokumen</a>
              </div>
          `;
          break;
  }
...
export { sendHandoverUpdateNotificationEmails };