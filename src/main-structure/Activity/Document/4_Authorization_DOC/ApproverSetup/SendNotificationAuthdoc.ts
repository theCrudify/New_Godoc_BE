// src/main-structure/Activity/Document/Authorization/ApproverChangeController.ts

import { prismaDB2 } from '../../../../../config/database';
import { sendEmail } from '../../../Email/EmailServiceEnvironment/EmailServiceExport'
// Create this as a separate file: src/main-structure/Activity/Email/ApproverChangeEmailAuthdoc.ts
// Then import these functions in your ApproverChangeController.ts

/**
 * Send email notification when approver change request is submitted for authorization documents
 */
export const sendApproverChangeRequestEmailAuthdoc = async (changeRequest: any, requesterDepartmentId?: number) => {
  try {
    console.log('📧 Sending authorization document approver change request email notification');
    console.log('🔍 DEBUG: Requester department_id:', requesterDepartmentId);

    const authdocNumber = changeRequest.tr_authorization_doc?.doc_number || 'Unknown Document';
    const requestId = changeRequest.id;
    const fromApprover = changeRequest.current_authorization?.employee_name || 'Unknown';
    const toApprover = changeRequest.new_authorization?.employee_name || 'Unknown';
    const requester = changeRequest.requester_authorization?.employee_name || 'Unknown';
    const reason = changeRequest.reason;
    const urgent = changeRequest.urgent;
    const step = changeRequest.tr_authdoc_approval?.step || 'Unknown';
    const actor = changeRequest.tr_authdoc_approval?.actor || 'Unknown';
    ...
    const subject = urgent 
      ? `[URGENT] Request Perubahan Approver - Authorization Document ${authdocNumber} (Request #${requestId})`
      : `Request Perubahan Approver - Authorization Document ${authdocNumber} (Request #${requestId})`;

    const emailTemplate = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px;">
          <h2 style="color: #333; margin-bottom: 20px;">
            ${urgent ? '🚨 ' : '📋 '}Request Perubahan Approver - Authorization Document
          </h2>
          ...
          <div style="background-color: white; padding: 20px; border-radius: 6px; margin-bottom: 20px;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
              ...
              <tr>
                <td style="padding: 8px 0; font-weight: bold;">Document Number:</td>
                <td style="padding: 8px 0;">${authdocNumber}</td>
              </tr>
              <tr>
    ...
    console.error('❌ Error sending authorization document approver change request email:', error);
    throw error;
  }
};

/**
 * Send email notification when approver change request is processed (approved/rejected) for authorization documents
 */
export const sendApproverChangeResultEmailAuthdoc = async (
  changeRequest: any, 
  status: 'approved' | 'rejected', 
  adminDecision: string
) => {
  try {
    console.log(`📧 Sending authorization document approver change result email (${status})`);

    const authdocNumber = changeRequest.tr_authorization_doc?.doc_number || 'Unknown Document';
    const requestId = changeRequest.id;
    const fromApprover = changeRequest.current_authorization?.employee_name || 'Unknown';
    const toApprover = changeRequest.new_authorization?.employee_name || 'Unknown';
    const requesterEmail = changeRequest.requester_authorization?.email;
    const requesterName = changeRequest.requester_authorization?.employee_name || 'Unknown';
    const reason = changeRequest.reason;
    const step = changeRequest.tr_authdoc_approval?.step || 'Unknown';
    const actor = changeRequest.tr_authdoc_approval?.actor || 'Unknown';
    ...
    const subject = `Request Perubahan Approver ${isApproved ? 'DISETUJUI' : 'DITOLAK'} - Authorization Document ${authdocNumber} (Request #${requestId})`;

    const emailTemplate = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px;">
          <h2 style="color: #333; margin-bottom: 20px;">
            ${isApproved ? '✅' : '❌'} Request Perubahan Approver ${isApproved ? 'Disetujui' : 'Ditolak'} - Authorization Document
          </h2>
    ...
    console.error('❌ Error sending authorization document approver change result email:', error);
    throw error;
  }
};

/**
 * Send email notifications when admin performs bypass operation for authorization documents
 */
export const sendBypassNotificationEmailsAuthdoc = async (
  authorizationDoc: any,
  affectedApprovals: any[],
  bypassReason: string,
  adminUser: any
) => {
  try {
    console.log('📧 Sending bypass notification emails to affected approvers for authorization document');

    const docNumber = authorizationDoc.doc_number || 'Unknown Document';
    const adminName = adminUser?.employee_name || 'System Admin';
    const bypassDate = new Date().toLocaleString('id-ID');

    const subject = `[URGENT] Sistem Approval Di-bypass - Authorization Document ${docNumber}`;
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px;">
            <h2 style="color: #dc3545; margin-bottom: 20px;">
              🚨 Pemberitahuan: Sistem Approval Di-bypass - Authorization Document
            </h2>
            ...
            <div style="background-color: white; padding: 20px; border-radius: 6px; margin-bottom: 20px;">
              ...
              <table style="width: 100%; border-collapse: collapse;">
                <tr>
                  <td style="padding: 8px 0; font-weight: bold; width: 30%;">Document Number:</td>
                  <td style="padding: 8px 0;">${docNumber}</td>
                </tr>
                <tr>
            ...
            <div style="background-color: #d1ecf1; padding: 15px; border-radius: 6px; margin-bottom: 20px;">
              <p style="margin: 0; color: #0c5460;">
                <strong>Informasi:</strong><br>
                Sistem approval untuk authorization document ini telah di-bypass oleh Super Admin. 
                Approval Anda tidak lagi diperlukan untuk document ini. 
                Document telah otomatis disetujui dan dapat dilanjutkan.
              </p>
            </div>
    ...
    // Also send summary email to document creator/submitter if available
    const submitterEmail = authorizationDoc.authorization?.email;
    if (submitterEmail) {
      const submitterTemplate = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px;">
            <h2 style="color: #28a745; margin-bottom: 20px;">
              ✅ Authorization Document Anda Telah Disetujui (Bypass)
            </h2>
            
            <div style="background-color: white; padding: 20px; border-radius: 6px; margin-bottom: 20px;">
              <h3 style="color: #007bff; margin-top: 0;">Detail Document</h3>
              <table style="width: 100%; border-collapse: collapse;">
                <tr>
                  <td style="padding: 8px 0; font-weight: bold; width: 30%;">Document Number:</td>
                  <td style="padding: 8px 0;">${docNumber}</td>
            ...
            <div style="background-color: #d4edda; padding: 15px; border-radius: 6px; margin-bottom: 20px;">
              <p style="margin: 0; color: #155724;">
                ...
                Authorization document Anda telah disetujui melalui bypass system oleh Super Admin. 
      ...
      await sendEmail({
        to: submitterEmail,
        subject: `✅ Authorization Document Disetujui (Bypass) - ${docNumber}`,
    ...
    console.error('❌ Error sending bypass notification emails for authorization document:', error);
    throw error;
  }
};
